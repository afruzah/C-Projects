...
void * writer(void * in)
{
	//instantiating char array called writing
	typedef char* (writing)[1000];
	
	//counting commands written
	int written = 0;
	
	//while loop to check if commands are written to
    while(written < total_commands)
    {
		if(get_written() == 0){
        	bad_write((*((writing*)in))[written]);
			//after written continues to write next command
			written++;
		}
    }
}
...
{
	//instantiating variable to count through until total commands
    int read = 0;
	//while variable read is less than total commands
	while(read < total_commands){
		//when program has finishing writing
		if(get_written() == 1){
        	bad_read(empty);
			//continues to read next command
			read++;
		}
    }
}
